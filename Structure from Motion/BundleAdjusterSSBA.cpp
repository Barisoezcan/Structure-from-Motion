...
int BundleAdjuster::Count2DMeasurements(const vector<measured_3d>& pointcloud) {
	int K = 0;
	for (unsigned int i = 0; i<pointcloud.size(); i++) {
		for (unsigned int ii = 0; ii<pointcloud[i].imgs_pnts.size(); ii++) {
			if (pointcloud[i].imgs_pnts[ii] >= 0) {
				K++;
			}
		}
	}
...
void BundleAdjuster::adjustBundle(vector<measured_3d>& pointcloud,
	Mat& cam_matrix,
	vector<Bild> bilder,
	vector<cv::Mat>& Pmats
)
{
	...
	{
		int pointId = j;
		Xs[j][0] = pointcloud[j].XYZ.x;
		Xs[j][1] = pointcloud[j].XYZ.y;
		Xs[j][2] = pointcloud[j].XYZ.z;
	...
	for (int i = 0; i < N; ++i)
	{
		...
		R[0][0] = P.at<double>(0, 0); R[0][1] = P.at<double>(0, 1); R[0][2] = P.at<double>(0, 2); T[0] = P.at<double>(0, 3);
		R[1][0] = P.at<double>(1, 0); R[1][1] = P.at<double>(1, 1); R[1][2] = P.at<double>(1, 2); T[1] = P.at<double>(1, 3);
		R[2][0] = P.at<double>(2, 0); R[2][1] = P.at<double>(2, 1); R[2][2] = P.at<double>(2, 2); T[2] = P.at<double>(2, 3);
	...
	{
		for (unsigned int i = 0; i<pointcloud[k].imgs_pnts.size(); i++) {
			if (pointcloud[k].imgs_pnts[i] >= 0) {
				int view = i, point = k;
				Vector3d p, np;

				Point cvp = bilder.at(i).keypoints.at(pointcloud[k].imgs_pnts[i]).pt;
				p[0] = cvp.x;
				p[1] = cvp.y;
				p[2] = 1.0;
	...
	if (good_adjustment) { //good adjustment?
		...
		for (unsigned int j = 0; j < Xs.size(); ++j)
		{
			...
			pointcloud[j].XYZ.x = Xs[j][0];
			pointcloud[j].XYZ.y = Xs[j][1];
			pointcloud[j].XYZ.z = Xs[j][2];
		}
