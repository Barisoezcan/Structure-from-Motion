...
static void getInputFromList(string _list_path, vector<string> &files, vector<Mat> &images, Mat &K, Mat &distCoeffs, string objectName)
{
	ifstream myfile(_list_path.c_str());

	if (!myfile.is_open())
	{
		cout << "Unable to read file: " << _list_path << endl;
		exit(0);
	}
	else
	{
		int found = _list_path.find_last_of("/\\");
		string line_str, path_to_file = _list_path.substr(0, found);

		int found2 = path_to_file.find_last_of("/\\");
		string object = path_to_file.substr(found2 + 1, path_to_file.size());
	...
	string camera_intrinsics = files.back();
	FileStorage fs(camera_intrinsics, FileStorage::READ);

	if (!fs.isOpened())
	{
		cout << "Could not open the configuration file: " << camera_intrinsics << endl;
		exit(0);
	}
	...
	if (DEBUG == 1)
	{
		cout << "K:" << endl << K << endl << endl << "distCoeffs: " << endl << distCoeffs << endl << endl;
	...
	{
		images.push_back(imread(files.at(i), CV_LOAD_IMAGE_UNCHANGED));
	}
...
static void help()
{
	cout
		<< "\n------------------------------------------------------------------------------------\n"
		<< " This program shows the Structure From Motion capabilities \n"
		<< " based on the OpenCV library module.\n"
		<< " It reconstructs a scene from a set of 2D images and camera calibration \n"
		<< " Usage:\n"
		<< "        main.exe <path_to_file> \n"
		<< " where: path_to_file is the file absolute path into your system which contains\n"
		<< "        the list of images to use for reconstruction \n"
		<< "        followed by the calibration file in yml format\n"
		<< "------------------------------------------------------------------------------------\n\n"
		<< endl;
}
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	//Liste mit Pfad zu Bildern und Kalibrierung
	string list_path;
	...
	{
		cout << "Bitte Pfad zur Liste mit den Dateien als Befehlsargument übergeben!" << endl;
		exit(0);
	}
	...
	//Bilder und Kamerakalibrierung einlesen
	vector<string> files;
	vector<Mat> images;
	Mat K, distCoeffs;
	string objectName;

	getInputFromList(list_path, files, images, K, distCoeffs, objectName);
	...
	{
		cout << "Bild: " << i << endl;
		Bild bild(i, images.at(i), K, distCoeffs);
